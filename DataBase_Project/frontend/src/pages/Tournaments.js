...
const Tournaments = () => {
  ...
  useEffect(() => {
    const fetchTeams = async () => {
      if (formData.leagueId) {
        try {
          const teamsData = await apiService.teams.getByLeague(formData.leagueId);
  ...
  return (
    <div className="max-w-7xl mx-auto">
      ...
      {/* Create/Edit Modal - Only render if admin */}
      {showModal && isAdmin && (
                ...
                <div className="grid grid-cols-2 gap-4">
                  ...
                  <div>
                    ...
                    <select
                      ...
                      onChange={(e) => setFormData({ ...formData, leagueId: e.target.value, selectedTeams: [] })}
                      className="w-full px-4 py-2 border border-gray-300 rounded-lg focus:ring-2 focus:ring-orange-500 focus:border-transparent text-gray-900 bg-white"
                    >
                      <option value="" className="text-gray-900">Select League</option>
                      {leagues.map(league => (
                        <option key={league.LeagueID || league.id} value={league.LeagueID || league.id} className="text-black">
                  ...
                  <div className="border border-gray-300 rounded-lg p-4 max-h-60 overflow-y-auto">
                    {teams.length > 0 ? (
                      ...
                      <div className="text-center text-gray-500 py-4">
                        {formData.leagueId ? 'No teams found in this league' : 'Select a league to view teams'}
                      </div>
                    )}
                  </div>
